  ...
  <div v-if="aplications.INDICADORES && screen.EJEINDICADOR.ver">
    ...
    <v-container>
      <v-row mt-20>
        ...
        <v-col>
          <v-btn color="warning" dark class="mb-2" @click="findMetasEsperadas"
            >Ejecución de la Meta</v-btn
          >
        </v-col>
                            ...
                            <v-row class="flex-column">
                              <v-col class="headline" cols="12">
                                ...
                                <v-text-field
                                  ...
                                  type="number"
                                  :disabled="esperar || !nowYear || temporal"
                                  outlined
                                  v-model="mes.metaEjecutada"
                                  @change="metaEjecutada"
                                ></v-text-field>
                              </v-col>
                              <v-card-actions>
                                <v-spacer></v-spacer>
                    ...
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn
                          ...
                          :disabled="!nowYear || temporal"
                          text
                          @click="saveMetasEjecutadasGeneral"
                          v-bind="attrs"
                          v-on="on"
                          >Guardar</v-btn
                        >
                ...
                <v-container>
                  ...
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <h5>Valor</h5>
                      <v-text-field
                        v-model="valorMes.metaEjecutada"
                        label="Valor"
                        type="number"
                      >
                      </v-text-field>
  ...
  methods: {
    ...
    verEjecucion(item) {
      ...
      axios
        .post(
          this.$api +
            this.$detalleEjecucionIndicador +
            "/getInforamtionByExecute",
          { id: item }
        )
        .then((response) => {
          var detalleEjecucionIndicador =
            response.data.detalleEjecucionIndicador[0];
          var user = response.data.user;
          var centros = response.data.centro;
          var com = 0;
          for (var i = 0; i < detalleEjecucionIndicador.length; i++) {
            var nameCenter =
              centros[i] != null ? centros[i].nombre : "Sin centro";
      ...
      if (lineaProgramatica != null && indicador != null && year != null) {
        await axios
          ...
          .then((response) => {
            ...
            for (var j = 0; j < 12; j++) {
              sumaMetaEsperada +=
                response.data.mes[j] != null
                  ? parseFloat(response.data.mes[j].valorejecucionesperada)
                  : 0;
            }
            ...
            if (sumaMetaEsperada > 0) {
              for (var j = 0; j < 12; j++) {
                this.meses[j].id =
                  response.data.mes[j] != null ? response.data.mes[j].id : null;
                this.meses[j].metaEsperada =
                  response.data.mes[j] != null
                    ? response.data.mes[j].valorejecucionesperada
                    : 0;
                if (response.data.metaEjecutada[j] != null) {
                  this.meses[j].centro_id =
                    response.data.metaEjecutada[j].centro_id;
                  this.meses[j].idDetalle = response.data.metaEjecutada[j].id;
                }
                this.meses[j].metaEjecutada =
                  response.data.mes != null
                    ? response.data.mes[j].valorejecucionrealizada
                    : 0;
              }
      ...
      if (this.editedIndex > -1) {
        ...
        for (var i = 0; i < this.meses.length; i++) {
          if (this.meses[i].valorMetaEsperada > 0) {
            ...
            this.editedItem.valorejecucionesperada =
              this.meses[i].valorMetaEsperada;
            this.editedItem.valorejecucionrealizada =
      ...
      for (var i = 0; i < 12; i++) {
        if (parseFloat(this.meses[i].metaEjecutada) < 0) {
          ...
          this.$swal({
            ...
            title: "Ups...!",
            text:
              "El valor de la meta ejecutada del mes " +
              this.meses[i].mes +
