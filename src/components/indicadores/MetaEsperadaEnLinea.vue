            ...
            <template v-slot:top>
                ...
                <v-container>
                    <v-row mt="20">
                        ...
                        <v-col sm="6" md="4" lg="2">
                            <v-text-field v-model="metaGlobal" label="Meta de la línea programatica" type="number" disabled></v-text-field>
                        </v-col>
                        <v-col sm="6" md="4" lg="2">
                            <v-btn color="warning" dark class="mb-2" @click="findMetasEsperadas">Metas Esperadas</v-btn>
                        </v-col>
                                                        ...
                                                        <v-card-text>
                                                            <v-row class="flex-column">
                                                                <v-col class="headline" cols="12">
                                                                    ...
                                                                    <v-text-field v-model.number="mes.valorMetaEsperada" label="valor" type="number" :disabled="esperar || nowYear"></v-text-field>
                                                                </v-col>
                                                            </v-row>
                                                        </v-card-text>
                                                    </v-card>
                                            ...
                                            <v-card-actions>
                                                ...
                                                <v-tooltip bottom>
                                                    <template v-slot:activator="{ on, attrs }">
                                                        <v-btn color="warning darken-1" :disabled="esperar || nowYear" text @click="save" v-bind="attrs" v-on="on">Guardar</v-btn>
                                                    </template>
                                                    <span>Guardar Nueva Ejecución</span>
                                                </v-tooltip>
                                                <v-tooltip bottom>
                                                    <template v-slot:activator="{ on, attrs }">
                                                        <v-btn color="warning darken-1" text @click="close" :disabled="esperar" v-bind="attrs" v-on="on">Cancelar</v-btn>
                                                    </template>
...
export default {
    ...
    computed: {
        ...
        formTitle() {
            return this.editedIndex === -1 ? 'Metas Esperadas' : 'Editar Metas Esperadas'
        },
    },
    mounted() {
        if (this.logged) {
    ...
    methods: {
        ...
        findMetasEsperadas() {
            ...
            if (lineaProgramatica != null && indicador != null && year != null) {
                ...
                axios
                    .post(this.$api + this.$metaEsperadaEnLinea + '/getMetasEsperadasPorLineaPorAnio', {
                    ...
                    .then((response) => {
                        for (var i = 0; i <= 11; i++) {
                            ...
                            this.meses[i].year = year
                            this.meses[i].valorMetaEsperada = response.data.mes[i] != null ? response.data.mes[i].valorejecucionesperada : 0
                            this.meses[i].valorejecucionrealizada = response.data.mes[i] != null ? response.data.mes[i].valorejecucionrealizada : 0
                        }
                        this.esperar = false
                    })
                this.dialog = true
