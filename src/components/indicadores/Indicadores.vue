        ...
        <v-toolbar flat color="white">
          ...
          <v-row class="mt-5">
            <v-col cols="12" sm="12" md="6">
              <v-select
                :items="years"
                :rules="[(v) => !!v || 'Debe seleccionar una año']"
                label="Año"
                @change="getIndicadores(yearSelected)"
              ...
              <v-container>
                <v-row>
                  ...
                  <v-col sm="12" md="6">
                    ...
                    <v-text-field
                      type="number"
                      v-model="newMetaGlobal.metaGlobal"
                    >
                    </v-text-field>
                  </v-col>
                </v-row>
              ...
              <v-container>
                <v-row>
                  ...
                  <v-col sm="8">
                    ...
                    <strong
                      v-if="
                        metaAcumulada < metaGlobal || metaAcumulada > metaGlobal
                      "
                      >Se ha ajustado la meta global del indicador, por lo que
                      se recomienda ajustar la asignación de las metas por
                      línea.</strong
                    >
                  </v-col>
                  <v-col sm="4">
                    <h5>Indicador</h5>
                    <strong v-if="rowForChange">{{
                      rowForChange.descripcion
                    }}</strong>
                  </v-col>
                  <v-col sm="4">
                    <h5>Meta Global</h5>
                    <strong v-if="rowForChange">{{
                      rowForChange.metaGlobal
                    }}</strong>
                  </v-col>
                  <v-col
                    sm="4"
                    :class="
                      metaAcumulada < metaGlobal
                        ? 'metaacumulada-menor'
                        : metaAcumulada > metaGlobal
                        ? 'metaacumulada-mayor'
                        : 'metaacumulada-igual'
                ...
                <v-row
                  v-for="lineaProgramatica of lineaProgramaticas"
                  :key="lineaProgramatica.id"
                >
                  <v-col
                    cols="12"
                    class="d-flex align-content-center mb-0 mt-0 p-0"
                  >
                    <v-col cols="4">
                      <v-checkbox
                        v-model="lineaProgramatica.selected"
                        :label="lineaProgramatica.linea"
                        color="red darken-3"
                      ></v-checkbox>
                    </v-col>
                    <v-col cols="2" class="d-flex align-content-center mt-0">
                      <input
                        v-model.number="lineaProgramatica.metaxlinea"
                        type="number"
                        single-line
                        hide-details
                    ...
                    >
                      ...
                      >
                        <template v-slot:activator="{ on, attrs }">
                          <v-btn
                            :class="
                              lineaProgramatica.metaxlinea > 0
                                ? ''
                                : 'v-btn--disabled'
                            "
                            color="primary"
                            ...
                            v-bind="attrs"
                            v-on="on"
                            @click="getCentros(lineaProgramatica)"
                          >
                        ...
                        <v-card>
                          <v-card-title class="warning" style="color: white">
                            <span class="headline"
                              >Meta por centro - {{ lineaSelected.linea }}</span
                            >
                          </v-card-title>
                          <v-card-text>
                            <v-container>
                                  ...
                                  >
                                    <v-col cols="6" class="text-center">
                                      ...
                                      <v-autocomplete
                                        ...
                                        v-model="mc.centro"
                                        @change="
                                          cambio(centros, mc.centro, index)
                                        "
                                    ...
                                    <v-col cols="6" class="text-center">
                                      <p>Meta</p>
                                      <v-text-field
                                        label="Meta aspiracion del centro"
                                        v-model.number="mc.metaCentro"
                                        :rules="[
                                          (v) =>
                                            !!v ||
                                            'La meta esperada es requerida.',
                                        ]"
                                        :error-messages="
                                          validarMeta(
                                            lineaSelected.metaxlinea,
                                            mc.metaCentro
                                          )
                                        "
                                        clearable
                          ...
                          <v-card-actions>
                            ...
                            <v-btn
                              color="blue darken-1"
                              text
                              @click="saveMetaLineaCentro(lineaSelected)"
                            >
                              Guardar
                            </v-btn>
                          </v-card-actions>
                ...
                <v-row>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn
                      :disabled="metaAcumulada != metaGlobal"
                      color="warning darken-1"
                      text
                      @click="estaSeguroDeGuardarLaInformacion"
                      >Guardar infomación
                    </v-btn>
                    <v-dialog
                      v-model="dialogEstaSeguroDeGuardarLaInformacion"
                      class="w-25"
                    >
                      <v-card>
                        <v-card-title
                          >¿Esta seguro de guardar la información?</v-card-title
                        >
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn
                            ...
                            :disabled="false"
                            text
                            @click="saveMetaPorLinea"
                            >SI</v-btn
                          >
                          <v-btn
                            color="warning"
                            text
                            @click="closeDialogEstaSeguroDeGuardarLaInformacion"
                            >NO</v-btn
                          >
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
                    <v-btn color="warning" text @click="closeMetaPorLinea"
                      >Cancelar</v-btn
                    >
                  </v-card-actions>
                </v-row>
              ...
              <v-container>
                ...
                <v-row>
                  <v-card-actions>
                    ...
                    <v-btn color="warning" text @click="closeNuevaMeta"
                      >Cancelar</v-btn
                    >
                  </v-card-actions>
                </v-row>
                ...
                <v-row>
                  <v-col cols="6" sm="6" md="6">
                    ...
                    <input
                      v-if="rowForChange"
                      type="text"
                      v-model="rowForChange.descripcion"
                    />
                  </v-col>
                </v-row>
                <v-row>
                  <v-card-actions>
                    ...
                    <v-btn color="warning" text @click="closeIndicador"
                      >Cancelar</v-btn
                    >
                  </v-card-actions>
                </v-row>
              ...
              <v-container>
                ...
                <v-row>
                  <v-card-actions>
                    ...
                    <v-btn color="warning" text @click="closeEstado"
                      >Cancelar</v-btn
                    >
                  </v-card-actions>
                </v-row>
                ...
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="6" sm="6" md="6">
                      <v-select
                        :items="clasificacion"
                        :rules="[
                          (v) => !!v || 'Debe seleccionar una clasificacion',
                        ]"
                        label="Clasificación"
                        @change="changedValueClasificacion"
                        v-model="clasificacionSeleccionada"
                      ></v-select>
                    </v-col>
                    <v-col cols="6" sm="6" md="6">
                      <v-select
                  ...
                  <v-row>
                    <v-col cols="6" sm="6" md="6">
                      <v-select
                        :items="periodicidad"
                        :rules="[
                          (v) => !!v || 'Debe seleccionar una periodicidad',
                        ]"
                        label="Periodicidad"
                        @change="changedValuePeriocidad"
                        v-model="periocidadSeleccionada"
                      ></v-select>
                    </v-col>
                    <v-col cols="6" sm="6" md="6">
                      <v-select
                        :items="plataforma"
                        :rules="[
                          (v) => !!v || 'Debe seleccionar una plataforma',
                        ]"
                        label="Plataforma"
                        @change="changedValuePlataforma"
                  ...
                  <v-row>
                    <v-col cols="6" sm="6" md="6">
                      <v-select
                        :items="unidad"
                        :rules="[(v) => !!v || 'Debe seleccionar una unidad']"
                        label="Unidad"
                        @change="changedValueUnidad"
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-bind="attrs"
              v-on="on"
              @click="asignacionMetaPorLinea(item)"
            >
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-bind="attrs"
              v-on="on"
              @click="ajusteALaMetaGlobal(item)"
            >
...
export default {
  ...
  mounted() {
    if (this.logged) {
      if (this.estado) {
        if (
          this.aplications.CONFIGURACION &&
          this.screen.ADMININDICADORES.ver
        ) {
  ...
  methods: {
    ...
    saveMetaLineaCentro(metaSelecionanda) {
      ...
      if (total <= meta && total > 0) {
        ...
        this.message =
          "La meta del centro(s) es mayor o menor ala meta de la linea";
        this.alert = true;
      }
      this.dialogLoading = false;
    },
    ...
    initialize() {
      ...
      axios
        ...
        .then((response) => {
          for (
            let index = 0;
            index < response.data.indicadores.length;
            index++
          ) {
            response.data.indicadores[index].estado =
              response.data.indicadores[index].estado === 1
                ? "Activo"
                : "Inactivo";
          }
          ...
          for (var item in response.data.indicadores) {
            ...
            this.parametros.push({
              ...
              clasificacion_id:
                response.data.indicadores[item].clasificacion_id,
              ...
              fechaUltimaModificacion:
                response.data.indicadores[item].fechaUltimaModificacion,
              usuarioCreacion: response.data.indicadores[item].usuarioCreacion,
              usuarioModificacion:
                response.data.indicadores[item].usuarioModificacion,
              ...
              metaGlobal:
                response.data.metaGlobal[item].metaGlobal == undefined
                  ? 0
                  : response.data.metaGlobal[item].metaGlobal,
            });
          }
        });
      ...
      axios
        ...
        .then((response) => {
          for (var item in response.data.data) {
            ...
            var elementolineaProgramaticas = this.lineaProgramaticas.findIndex(
              (x) => x.value == elemento.linea_id
            );
            var elementoSeleccionado =
              this.lineaProgramaticas[elementolineaProgramaticas];
    ...
    deleteItem() {
      ...
      if (this.parametros.splice(index, 1)) {
        axios
          .delete(this.$api + this.$indicadores + "/" + this.deletedItem.id)
          .then((response) => {
    ...
    saveEstado() {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.dialogEstado = false;
          this.initialize();
          var elemento = this.parametros.findIndex(
            (x) => x.id == this.indicador_id
          );
    ...
    getIndicadores(year) {
      ...
      axios
        .get(
          this.$api + this.$indicadorMetaAno + "/getIndicadoresYears/" + year
        )
        .then((response) => {
          for (
            let index = 0;
    ...
    saveMetaGLobal() {
      ...
      this.$swal({
        ...
        axios
          .post(
            this.$api + this.$indicadorMetaAno + "/createMetaGlobalYear",
            this.newMetaGlobal
          )
          .then((response) => {
              ...
              .then((response) => {
                ...
                for (var item in response.data.indicadores) {
                  ...
                  this.parametros.push({
                    id: response.data.indicadores[item].id,
                    clasificacion:
                      response.data.indicadores[item].clasificacion,
                    clasificacion_id:
                      response.data.indicadores[item].clasificacion_id,
                    categoria: response.data.indicadores[item].categoria,
                    categoria_id: response.data.indicadores[item].categoria_id,
                    cliente_id: response.data.indicadores[item].cliente_id,
                    plataforma_id:
                      response.data.indicadores[item].plataforma_id,
                    periodicidad_id:
                      response.data.indicadores[item].periodicidad_id,
                    unidad_id: response.data.indicadores[item].unidad_id,
                    descripcion: response.data.indicadores[item].descripcion,
                    estado: response.data.indicadores[item].estado,
                    fechaCreacion:
                      response.data.indicadores[item].fechaCreacion,
                    fechaUltimaModificacion:
                      response.data.indicadores[item].fechaUltimaModificacion,
                    usuarioCreacion:
                      response.data.indicadores[item].usuarioCreacion,
                    usuarioModificacion:
                      response.data.indicadores[item].usuarioModificacion,
                    created_at: response.data.indicadores[item].created_at,
                    periodicidad: response.data.indicadores[item].periodicidad,
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        this.editedItem.usuarioModificacion =
          this.usuarioModificacionSeleccionado;
        ...
        axios
          ...
          .then((response) => {
            ...
            this.editedItem.estado =
              response.data.data.estado === true ? "Activo" : "Inactivo";
    ...
    getYearsIndi() {
      axios
        .post(this.$api + this.$indicadorMetaAno + "/getYears")
        .then((response) => {
          this.dialogLoading = true;
