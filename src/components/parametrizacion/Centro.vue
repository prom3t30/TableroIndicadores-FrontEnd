<template>
    <div v-if="aplications.PARAMETRIZACION && screen.CENTROS.ver">
        <v-data-table :headers="headers" :items="parametros" :search="search" sort-by="valor" class="elevation-1">
            <template v-slot:top>
                <!--INICIO Seccion de avisos y alertas de centro-->
                ...
                <!--FIN Seccion de avisos y alertas de la centro-->
                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nuevo centro</v-btn>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Centro" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                    ...
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-select :items="responsable" :rules="[(v) => !!v || 'Debe seleccionar un responsable']" label="Responsable centro" hint="Subdirector a cargo del centro" v-model="editedItem.ResponsableCentro" required></v-select>
                                        </v-col>
                                    </v-row>
                                </v-container>
                            </v-card-text>
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: 'Responsable Centro', value: 'name', sortable: false },
    ...
    methods: {
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$centros + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$centros, this.editedItem)
                    .then((response) => {
                ...
                if (this.editedItem.Nombre && this.editedItem.ResponsableCentro) {
                    /*
          console.log(this.editedItem.Nombre);
          console.log(this.editedItem.ResponsableCentro);
          console.log(this.editedItem.Descripcion);
        */

                    axios
                        .post(this.$api + this.$centros, this.editedItem)
                        .then((response) => {
                        ...
                        .catch((err) => {
                            this.alert = true
                            this.message = err.response.data.errors.Nombre[0] + err.response.data.errors.responsable[0]
                            this.dialogLoading = false
                            this.close()
                        })
                } else {
                    this.alert = true
                    this.message = 'Los campos nombre y responsable Centro son requeridos'
                    this.dialogLoading = false
                }
            }
        },
