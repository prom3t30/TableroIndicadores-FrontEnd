<template>
    <div v-if="aplications.CONFIGURACION && screen.FUNCIONES.ver">
        <v-data-table :headers="headers" :items="parametros" :search="search" sort-by="valor" class="elevation-1">
            <template v-slot:top>
                <!--INICIO Seccion de avisos y alertas de la funcion-->
                ...
                <!--FIN Seccion de avisos y alertas de la funcion-->
                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva función</v-btn>
                        ...
                        <v-card>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Funcion" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                            ...
                            <v-card-actions>
                                ...
                                <v-btn color="warning darken-1" :disabled="disabled" text @click="save">Guardar</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-dialog>
                </v-toolbar>
...
export default {
    ...
    computed: {
        ...
        formTitle() {
            return this.editedIndex === -1 ? 'Nueva función' : 'Editar función'
        },
        disabled() {
    ...
    methods: {
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$funciones + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$funciones, this.editedItem)
                    .then((response) => {
                ...
                axios
                    .post(this.$api + this.$funciones, this.editedItem)
                    .then((response) => {
