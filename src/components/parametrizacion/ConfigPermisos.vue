        ...
        <div>
            ...
            <v-snackbar v-model="alert" :timeout="timeout">
                ...
                <template v-slot:action="{ attrs }">
                    <v-btn color="blue" text v-bind="attrs" @click="alert = false">Cerrar</v-btn>
                </template>
            </v-snackbar>
            ...
            <v-card class="mr-3 ml-3">
                <v-toolbar-title class="ml-3">Configuración de roles y permisos</v-toolbar-title>
                <v-toolbar flat color="white">
                    <v-select class="mt-6" :items="aplicaciones" label="Aplicaciones" @change="changedValueAplicacionSeleccionada"></v-select>
                    <v-spacer></v-spacer>
                    <v-select class="mt-6" :items="roles" label="Roles" @change="changedValueRolSeleccionada"></v-select>
                </v-toolbar>
                <div class="text--primary ml-3">En este módulo se realizan las configuracion de permisos para las aplicaciones.</div>
                <v-container>
                    <v-row>
                        <v-col sm="6" md="6" lg="6">
                            <v-card class="pl-5 pt-3">
                                <h3>Pantallas de la aplicación</h3>
                                <h1 v-if="pantallasFiltradas.length == 0">Seleccione una aplicación</h1>
                                <v-radio-group v-model="pantallaSeleccionada">
                                    <v-radio v-for="item in pantallasFiltradas" :key="item.value" :label="`${item.text}`" :value="item.value" @click="changedValuePantallaSeleccionada"></v-radio>
                                </v-radio-group>
                            </v-card>
                        </v-col>
                        ...
                        <v-col sm="6" md="6" lg="6">
                            <v-card class="pl-5 pt-3">
                                <h3>Privilegios</h3>
                                <v-checkbox class="mt-0 mb-0" v-for="item in funciones" :key="item.value" :label="`${item.text}`" :value="item.value" v-model="privilegioSeleccionado" @click="changedPrivilegioSeleccionado"></v-checkbox>
                                <v-btn class="ma-2" tile color="success" @click="actualizarPrivilegios"> <v-icon left>mdi-pencil</v-icon>Actualizar privilegios </v-btn>
                            </v-card>
                        </v-col>
                    </v-row>
                </v-container>
    ...
    methods: {
        ...
        changedValuePantallaSeleccionada: function () {
            if (this.rolSeleccionado === null) {
                ...
                this.dialogLoading = true
                axios.get(this.$api + this.$privilegios + '/getprivilegios/' + this.aplicacionFiltrada + '/' + this.pantallaSeleccionada + '/' + this.rolSeleccionado).then((response) => {
                    console.log(response.data.data)
                    this.privilegioSeleccionado = []
                    for (var index in response.data.data) {
                        if (response.data.data[index].estado === 1) {
                            this.privilegioSeleccionado.push(response.data.data[index].funcion_id)
                        }
                    }
                    this.dialogLoading = false
                })
        ...
        initializeAplicaciones: function () {
            this.dialogLoading = true
            axios.get(this.$api + this.$pantallas + '/indexById/' + this.aplicacionFiltrada).then((response) => {
                this.dialogLoading = false
