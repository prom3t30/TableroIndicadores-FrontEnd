                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva pantalla</v-btn>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Pantalla" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-select :items="aplicaciones" :rules="[(v) => !!v || 'Debe seleccionar una aplicación']" label="Aplicaciones" @change="changedValueAplicacion" v-model="aplicacionSeleccionada"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
...
export default {
    ...
    methods: {
        ...
        initialize() {
            ...
            axios
                .get(this.$api + this.$pantallas + '/indexById/' + this.aplicacionFiltrada)
                .then((response) => {
                    this.parametros = response.data.data
                    this.dialogLoading = false
                })
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$pantallas + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            if (this.aplicacionSeleccionada != null) this.editedItem.aplicacion_id = this.aplicacionSeleccionada
            else {
            ...
            if (this.editedIndex > -1) {
                ...
                axios
                    ...
                    .then((response) => {
                        ...
                        this.editedItem.aplicacion.Nombre = this.aplicaciones.filter((x) => x.value === this.aplicacionSeleccionada).map((x) => x.text)
