<template>
    <div v-if="aplications.PARAMETRIZACION && screen.UNIDAD.ver">
        <v-data-table :headers="headers" :items="parametros" :search="search" sort-by="valor" class="elevation-1">
            <template v-slot:top>
                <!--INICIO Seccion de avisos y alertas de la unidad-->
                ...
                <!--FIN Seccion de avisos y alertas de la unidad-->
                ...
                <v-toolbar flat color="white">
                    <v-toolbar-title>Unidad </v-toolbar-title>
                    <v-spacer></v-spacer>
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva unidad</v-btn>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Unidad" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
...
export default {
    ...
    mounted() {
        if (this.logged) {
            if (this.estado) {
                if (this.aplications.PARAMETRIZACION && this.screen.UNIDAD.ver) {
    ...
    methods: {
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$unidad + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$unidad, this.editedItem)
                    .then((response) => {
                ...
                axios
                    .post(this.$api + this.$unidad, this.editedItem)
                    .then((response) => {
