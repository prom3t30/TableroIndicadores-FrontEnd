<template>
    <div v-if="aplications.PARAMETRIZACION && screen.LINEA.ver">
        ...
        <!-- SI tableState TIENE ALGUN REGISTRO SE MOSTRARA LA PAGINA -->
        <div v-if="tableState">
            <v-data-table :headers="headers" :items="parametros" :search="search" sort-by="valor" class="elevation-1">
                <template v-slot:top>
                    <!--INICIO Seccion de avisos y alertas de la lineaprogramatica-->
                    ...
                    <!--FIN Seccion de avisos y alertas de la lineaprogramatica-->
                    ...
                    <v-toolbar flat color="white">
                        <v-toolbar-title>Linea programatica </v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <v-dialog v-model="dialog" max-width="500px">
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva Línea programática</v-btn>
                                    ...
                                    <v-container>
                                        <v-row>
                                            <v-col cols="12" sm="12" md="12">
                                                <v-text-field v-model="editedItem.Nombre" label="Lineaprogramatica" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                            </v-col>
                                        </v-row>
                                        <v-row>
                                            <v-col cols="12" sm="12" md="12">
                                        ...
                                        <v-row>
                                            <v-col cols="12" sm="12" md="12">
                                                <v-select :items="responsable" :rules="[(v) => !!v || 'Debe seleccionar un responsable']" label="Responsable línea" v-model="editedItem.ResponsableLinea" required></v-select>
                                            </v-col>
                                        </v-row>
                                    </v-container>
                                </v-card-text>
...
export default {
    data: () => ({
        ...
        headers: [
            {
                text: 'Linea programatica',
                align: 'start',
                sortable: true,
                value: 'Nombre',
            },
            { text: 'Responsable línea', value: 'responsableNombre' },
    ...
    mounted() {
        if (this.logged) {
            if (this.estado) {
                if (this.aplications.PARAMETRIZACION && this.screen.LINEA.ver) {
                    this.initialize()

                    axios
                        .get(this.$api + this.$usuarios + '/getUsersManagerLine')
                        .then((response) => {
                            this.responsable = []
    ...
    computed: {
        ...
        formTitle() {
            return this.editedIndex === -1 ? 'Nueva linea programatica' : 'Editar línea programatica'
        },
        disabled() {
    ...
    methods: {
            ...
            axios.get(this.$api + this.$lineaprogramatica).then((response) => {
                for (var item in response.data.data) {
                    this.parametros.push({
                        ...
                        Nombre: response.data.data[item].Nombre,
                        Descripcion: response.data.data[item].Descripcion,
                        ResponsableLinea: response.data.data[item].ResponsableLinea,
                        responsableNombre: response.data.data[item].name,
                    })
                }
            })
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$lineaprogramatica + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$lineaprogramatica, this.editedItem)
                    .then((response) => {
                        ...
                        /* axios
              .get(this.$api+this.$usuarios+'/getUser/'+this.editedItem.ResponsableLinea)
              .then((response) => {
                this.editedItem.ResponsableLinea= response.data.name
                ...
                if (this.editedItem.Nombre != '') {
                    if (this.editedItem.Descripcion != '') {
                        if (this.editedItem.ResponsableLinea > 0) {
                            axios
                                .post(this.$api + this.$lineaprogramatica, this.editedItem)
                                .then((response) => {
                            ...
                            this.$swal({
                                icon: 'error',
                                title: 'Ups...!',
                                text: 'No ha escogido un responsable de línea',
                            })
                        }
                    } else {
                        this.dialogLoading = false
                        this.$swal({
                            icon: 'error',
                            title: 'Ups...!',
                            text: 'No digito una descripción de la línea',
                        })
                    }
                } else {
                    this.dialogLoading = false
                    this.$swal({
                        icon: 'error',
                        title: 'Ups...!',
                        text: 'No digito un nombre para la línea',
                    })
                }
            }
        },
