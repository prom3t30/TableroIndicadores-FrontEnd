<template>
    <div v-if="aplications.PARAMETRIZACION && screen.CLASIFICACION.ver">
        <v-data-table :headers="headers" :items="parametros" :search="search" sort-by="valor" class="elevation-1">
            <template v-slot:top>
                <!--INICIO Seccion de avisos y alertas de clasificacion-->
                ...
                <!--FIN Seccion de avisos y alertas de la clasificacion-->
                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva Clasificación</v-btn>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Clasificación" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
...
export default {
    ...
    mounted() {
        if (this.logged) {
            if (this.estado) {
                if (this.aplications.PARAMETRIZACION && this.screen.CLASIFICACION.ver) {
    ...
    computed: {
        ...
        formTitle() {
            return this.editedIndex === -1 ? 'Nueva clasificacion' : 'Editar clasificacion'
        },
        disabled() {
    ...
    methods: {
        initialize() {
            ...
            axios
                .get(this.$api + this.$clasificacion)
                .then((response) => {
                    this.parametros = response.data.data
                    this.dialogLoading = false
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$clasificacion + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$clasificacion, this.editedItem)
                    .then((response) => {
                ...
                axios
                    .post(this.$api + this.$clasificacion, this.editedItem)
                    .then((response) => {
