<template>
  <div v-if="aplications.CONFIGURACION && screen.USUARIOS.ver">
    <v-data-table
      ...
      class="elevation-1"
      :loading="cargando"
      loading-text="Cargando usuarios de la base de datos..."
    >
      <template v-slot:top>
        ...
        <v-toolbar flat color="white">
          ...
          <v-select
            ...
            :items="roles"
            label="Rol"
            @change="changedValueRolSeleccionado"
          ></v-select>
          <v-spacer></v-spacer>
          <v-select
            class="mt-6"
          ...
          <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card>
              ...
              <v-card-text>
                <v-container>
                  ...
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        ...
                        :rules="[
                          (v) => !!v || 'Debe digitar el número de documento',
                        ]"
                      ></v-text-field>
                    </v-col>
                  </v-row>
                  ...
                  <v-row>
                    <v-col cols="12" sm="12" md="12">
                      <v-text-field
                        v-model.number="editedItem.centro"
                        hint="CentroDiligencie el numero del centro al que pertenece"
                        type="number"
                        required
                        label="Diligencie el numero del centro"
                      ></v-text-field>
                    </v-col>
                  </v-row>
                </v-container>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="warning darken-1" text @click="close"
                  >Cancelar</v-btn
                >
                <v-btn
                  color="warning darken-1"
...
export default {
  ...
  computed: {
    ...
    disabled() {
      return (
        this.editedItem.Nombree === "" ||
        this.editedItem === "" ||
        this.editedItem.documento == 0
      ); // or === 0
    },
  },
  ...
  methods: {
    ...
    changedValueRolSeleccionado: function (value) {
      ...
      this.parametros = [];
      axios
        .get(this.$api + this.$usuarios + "/getbyrolyaplicacion/" + value)
        .then((response) => {
          for (var i = 0; i < response.data.data.length; i++) {
            this.parametros.push({
    ...
    deleteItem() {
      ...
      if (this.parametros.splice(index, 1)) {
        axios
          .delete(this.$api + this.$usuarios + "/" + this.deletedItem.id)
          .then((response) => {
    ...
    save() {
      if (this.rolSeleccionado != null)
        this.editedItem.rol_id = this.rolSeleccionado;
      else {
      ...
      if (this.editedIndex > -1) {
        ...
        axios
          ...
          .then((response) => {
            ...
            this.editedItem.rol.nombre = this.aplicaciones
      ...
      if (name.length > 0) {
        if (name.length > 0) {
          if (
            ...
            if (
              this.editedItem.documento > 0 &&
              parseFloat(this.editedItem.documento) % 1 == 0
            ) {
              if (this.editedItem.rol_id > 0) {
                if (email.length > 0) {
                  if (email.includes("@sena.edu.co")) {
                    this.editedItem.email = this.editedItem.email.replace(
                      / /g,
                      ""
                    );
