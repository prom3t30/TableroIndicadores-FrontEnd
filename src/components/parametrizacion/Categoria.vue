<template>
    <div v-if="aplications.PARAMETRIZACION && screen.CATEGORIAS.ver">
                ...
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="warning" dark class="mb-2" v-bind="attrs" v-on="on">Nueva categoría</v-btn>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
                                            <v-text-field v-model="editedItem.Nombre" label="Categoría" :rules="[(v) => !!v || 'Debe digitar un Nombre']"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12" sm="12" md="12">
...
export default {
    ...
    mounted() {
        if (this.logged) {
            if (this.estado) {
                if (this.aplications.PARAMETRIZACION && this.screen.CATEGORIAS.ver) {
    ...
    methods: {
        ...
        deleteItem() {
            ...
            if (this.parametros.splice(index, 1)) {
                axios.delete(this.$api + this.$categorias + '/' + this.deletedItem.id).then((response) => {
        ...
        save() {
            ...
            if (this.editedIndex > -1) {
                axios
                    .put(this.$api + this.$categorias, this.editedItem)
                    .then((response) => {
                ...
                axios
                    .post(this.$api + this.$categorias, this.editedItem)
                    .then((response) => {
