<template>
    <v-row class="fill-height" v-if="aplications.EVENTOS && screen.GESTIONEVENTOS.ver">
        ...
        <v-dialog v-model="dialog" @click:outside=";(nuevo = false), (editar = false), (eliminar = false)" max-width="500px">
            <!-- CARD PARA NUEVA CATEGORIA -->
            <v-card v-if="nuevo">
                <v-card-title class="warning text-white"> Nueva Categoria de evento </v-card-title>
                <v-card-text>
                    <v-container fluid>
                        <v-text-field label="Nombre:" hint="Nombre de la categoria" name="categoria" v-model="editEventoCategoria.nombre" autofocus :rules="nombreEvento"></v-text-field>
                        <v-textarea label="Descripcion:" hint="Descripcion de la categoria del evento" v-model="editEventoCategoria.descripcion" :rules="C_textarea"></v-textarea>
                    </v-container>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click=";(dialog = false), (nuevo = false)" text color="warning darken-1">Cancelar</v-btn>
                    <v-btn @click="saveCategory" text color="warning darken-1">Guardar</v-btn>
                </v-card-actions>
            </v-card>

            <!-- CARD PARA EDITAR -->
            <v-card v-else-if="editar">
                <v-card-title class="warning text-white"> Editar categoria del evento </v-card-title>
                <v-card-text>
                    <v-container fluid>
                        <v-text-field label="Nombre:" hint="Nombre de la categoria" name="categoria" v-model="editItemCategoria.nombre" autofocus :rules="nombreEvento"></v-text-field>
                        <v-textarea label="Descripcion:" hint="Descripcion de la categoria del evento" v-model="editItemCategoria.descripcion" :rules="C_textarea"></v-textarea>
                    </v-container>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click=";(dialog = false), (editar = false)" text color="warning darken-1">Cancelar</v-btn>
                    <v-btn @click="uploadCategoriaEvento" text color="warning darken-1">Guardar</v-btn>
                </v-card-actions>
            </v-card>
            <!-- CARD PARA EDITAR -->
            ...
            <!-- CARD PARA ELIMINAR -->
            <v-card v-else>
                <v-card-title class="headline"> Eliminar categoria de evento </v-card-title>
                <v-card-text>Desea borrar la categoria?</v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click=";(dialog = false), (eliminar = false)" text color="warning darken-1"> Cancelar </v-btn>
        ...
        <v-col>
            <v-toolbar flat color="white">
                ...
                <v-spacer></v-spacer>
                <v-btn color="orange text-white" @click=";(dialog = true), (nuevo = !nuevo)"> Nueva Categoria </v-btn>
            </v-toolbar>
            <v-data-table :headers="headers" :items="items" :search="search" :items-per-page="5" class="elevation-1">
                <template v-slot:[`item.actions`]="{ item }">
                    ...
                    <v-btn icon color="error" small class="mr-2" @click="deleteItem(item)">
            ...
            try {
                this.$swal({
                    ...
                    if (result.isConfirmed) {
                        this.dialog = false
                        axios
                            .delete(this.$api + this.$categoriaevento + '/' + this.editItemCategoria.id)
                            .then((res) => {
                                if (res.status == 204) {
                                    this.getCategorys()
                                    this.eliminar = false
                                    this.$swal('Eliminada!', 'La categoria ha sido eliminada.', 'success')
                                } else {
                                    this.$swal('No fue eliminada!', 'La categoria no fue eliminada.', 'warninig')
                                }
                            })
                            .catch((err) => {
                                console.error(err)
