<template>
  <v-row
    class="fill-height"
    v-if="aplications.EVENTOS && screen.GESTIONEVENTOS.ver"
  >
    ...
    <v-col>
      ...
      <v-sheet height="535">
        ...
        <v-dialog v-model="dialog" max-width="600px">
          <v-card>
            <v-card-title class="text-h5 orange text-white">
              Agregar evento
            </v-card-title>
            ...
            <v-container fluid class="mt-n2 mb-0">
              <v-form ref="form" @submit.prevent="addEvent">
                <v-text-field
                  name="nombre"
                  v-model="editEvento.nombre"
                  label="Nombre del eveto"
                  textarea
                  autofocus
                  class="m-0"
                  counter="100"
                ...
                <v-textarea
                  name="descripcion"
                  v-model="editEvento.descripcion"
                  label="Descripcion"
                ...
                <v-select
                  ...
                  @change="categoriaSeleccionada"
                ></v-select>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn text color="orange" @click="dialog = false"
                    >Cancelar</v-btn
                  >
                  <v-btn
                    type="submit"
        ...
        >
          ...
          >
              ...
              <v-menu bottom right>
                ...
                <v-list>
                  <v-container grid-list-sm>
                    <v-switch
                      v-model="selectedEvent.estado"
                      :color="selectedEvent.color"
                      :label="selectedEvent.estado ? 'Ocultar' : 'Publicar'"
                      inset
                      prepend-icon="mdi-send-check"
                      :persistent-hint="true"
                      hint="Ocultar/Publiacar"
                      @change="publicEvent(selectedEvent)"
                    ></v-switch>
                  </v-container>
                </v-list>
              </v-menu>
            ...
            <v-card-text v-if="currentlyEditing !== selectedEvent.id">
              <div
                v-if="selectedEvent.poster"
                class="font-weight-bold container-fluid"
              >
                Poster:
                <a
                  :href="this.rutaImg + this.selectedEvent.poster"
                  target="_blank"
                ...
                >
                  <v-img
                    ...
                    class="card"
                    alt="poster"
                    :src="this.rutaImg + this.selectedEvent.poster"
                  ></v-img>
                </a>
              </div>
              <v-card-text>
                ...
                <div class="mb-2">
                  <span class="font-weight-bold">Fecha inicio / Fin:</span
                  ><br />
                ...
                <v-chip
                  ><a
                    class="card-link flex-wrap"
                    target="_blank"
                    :href="'http://' + selectedEvent.link"
                    >{{ selectedEvent.link }}</a
                  ></v-chip
                >
              ...
              <v-card-actions class="mt-n2">
                <v-spacer></v-spacer>
                <v-btn text color="orange" @click="selectedOpen = false">
                  Cerrar
                </v-btn>
              </v-card-actions>
            </v-card-text>
            ...
            <v-form v-else>
              <v-container fluid>
                ...
                <v-text-field
                  name="ubicacion"
                  v-model="selectedEvent.ubicacion"
                  append-icon="mdi-map-marker"
                  label="Ubicacion"
                  textarea
                  class="mt-0 mb-0"
                ...
                <v-row class="mt-n5">
                  ...
                  <v-col cols="3">
                    <v-img
                      v-if="this.editEvento.poster"
                      class="img-fluid"
                      :src="this.editEvento.poster"
                      height="100"
                      width="100%"
                    ></v-img>
                    <v-img
                ...
                <v-select
                  :items="items"
                  :label="selectedEvent.categoria"
                  v-model="selectedEvent.categoria"
                  @change="editCategoriaSeleccionada"
                  hint="Seleccione una categoria"
                  :persistent-hint="true"
                ></v-select>
                <v-card-actions class="mt-n1">
                  <v-spacer></v-spacer>
                  <v-btn text color="orange" @click="cancelEdit">
                    Cancelar
                  </v-btn>
                  <v-btn
                    text
...
export default {
  ...
  data: () => ({
    ...
    fecha: [
      (value) => !!value || "Este campo es nesesario.",
      (value) => (value || "").length <= 50 || "La fecha es muy larga",
    ],
    focus: "",
    type: "month",
    typeToLabel: {
    ...
    async addEvent() {
      try {
        if (
          ...
          this.dialogLoading = true;
          this.editEvento.fechaInicio = moment(
            this.editEvento.fechaInicio
          ).format("YYYY-MM-DD HH:mm:ss");
          this.editEvento.fechaFin = moment(this.editEvento.fechaFin).format(
            "YYYY-MM-DD HH:mm:ss"
          );
          ...
          this.message =
            "Los campos nombre del evento fecha inicial, final y categoria son requeridos";
          this.alert = true;
        }
      } catch (error) {
        console.error(error);
